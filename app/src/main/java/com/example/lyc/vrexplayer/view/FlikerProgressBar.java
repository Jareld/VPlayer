        ...
        implements Runnable
{
    ...
    private void initPgBimap() {
        Log.d(TAG, "initPgBimap: 进入来");
        Log.d(TAG, "initPgBimap: "+Thread.currentThread());
        Log.d(TAG, "initPgBimap: 赋值前" + needWidth +"::" + needHeight);

        if (needWidth == 0 && needHeight == 0) {
            needWidth = getMeasuredWidth() - borderWidth;
            needHeight = getMeasuredHeight() - borderWidth;
            Log.d(TAG, "initPgBimap: 进入了赋值1");
        }

        if(needWidth == 1374 && needHeight == 66){
            needWidth = getMeasuredWidth() - borderWidth;
            needHeight = getMeasuredHeight() - borderWidth;
        }
        ...
        if (needWidth <= 0 || needHeight <= 0 ){
            needWidth = 1374;
            needHeight = 66;
            Log.d(TAG, "initPgBimap: 进入了赋值2");
            Log.d(TAG, "initPgBimap: 这是里因为宽高为负数了 赋值后"+ needWidth +"::" + needHeight);
        }
        Log.d(TAG, "initPgBimap: 赋值后" + needWidth +"::" + needHeight);
