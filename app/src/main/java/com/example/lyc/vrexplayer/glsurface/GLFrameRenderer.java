...
/*
 *  @项目名：  TestLocalSocket 
 *  @包名：    com.example.lyc2.testlocalsocket
 *  @文件名:   GLFrameRenderer
 *  @创建者:   LYC2
 *  @创建时间:  2016/12/20 14:45
 *  @描述：    TODO
 */
    ...
    public void update(int w, int h) {
        if (w > 0 && h > 0) {
            ...
            if (mScreenWidth > 0 && mScreenHeight > 0) {
                float f1 = 1f * mScreenWidth / mScreenHeight;
                float f2 = 1f * h / w;
                if (f1 == f2) {
                    LogUtils.logInfo(TAG, "updatewh", "::::f1==f2");
...
//                                                         1.0f, });
                    prog.createBuffers(new float[] { -1.0f, -1.0f, 1.0f, -1.0f, -1.0f, 1.0f, 1.0f,
                                                     1.0f, });
                    LogUtils.logInfo(TAG, "updatewh", "::::f1< f2");
